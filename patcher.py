...
class Patcher(object):
    ...
    def patch_exe(self):
        ...
        with io.open(self.executable_path, "r+b") as fh:
            ...
            match_injected_codeblock = re.search(rb"\$cdc.*", new_content)
            if match_injected_codeblock:
                target_bytes = match_injected_codeblock[0][:-2]
                new_target_bytes = (
                    b'rmaweasdjflasutopfhvcZLmtghj'.ljust(
                        len(target_bytes), b" "
                    )
                )

                new_content1 = new_content.replace(target_bytes, new_target_bytes)

                if new_content == new_content1:
                    logger.warning(
                        "something went wrong patching the driver binary. could not find injection code block"
                    )
                else:
